...
function buildCodebook(dirBits: 8 | 9): Vec3[] {
...
function packU9(values: Uint16Array): Uint8Array {
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    for (let b = 0; b < 9; b++) {
      if (v & (1 << b)) out[bitPos >> 3] |= 1 << (bitPos & 7);
      bitPos++;
    }
  }
  return out;
...
function unpackU9(packed: Uint8Array, count: number): Uint16Array {
...
function toU16BE(values: Int16Array): Uint8Array {
...
function fromU16BE(buf: Uint8Array, count: number): Int16Array {
  if (buf.length !== count * 2) throw new Error('Residual stream size mismatch');
...
function extractBlock(width: number, rgb: Uint8Array, r: BlockRect): Uint8Array {
...
function placeBlock(width: number, rgb: Uint8Array, r: BlockRect, blk: Uint8Array): void {
  for (let y = 0; y < r.h; y++) {
...
function luminanceVariance(blk: Uint8Array): number {
...
function gradientEnergy(blk: Uint8Array, w: number, h: number): number {
  if (w < 2 || h < 2) return 0;
  let sum = 0;
  let n = 0;
  for (let y = 0; y < h - 1; y++) {
...
function laplacianEnergyLuma(blk: Uint8Array, w: number, h: number): number {
  if (w < 3 || h < 3) return 0;
  let sum = 0;
  let n = 0;
  for (let y = 1; y < h - 1; y++) {
...
function classifyBlock(blk: Uint8Array, w: number, h: number): 'flat' | 'edge' | 'noise' {
  ...
  if ((v > 1800 && g > 32 && lap > 22) || (v > 2200 && g > 28) || lap > 38) return 'noise';
  if (g > 18) return 'edge';
  return 'flat';
}

function encodeVectorCore(width: number, height: number, rgb: Uint8Array, dirBits: 8 | 9): Uint8Array {
  ...
  let dirRaw: Uint8Array;
  if (dirBits === 8) {
    dirRaw = new Uint8Array(pixels);
    for (let i = 0; i < pixels; i++) dirRaw[i] = dirU16[i]!;
  } else {
...
function encodeClassicCore(width: number, height: number, rgb: Uint8Array): Uint8Array {
...
function encodePlaneStackCore(width: number, height: number, rgb: Uint8Array): Uint8Array {
...
function decodePlaneStack(width: number, height: number, stacked: Uint8Array): Uint8Array {
  const n = width * height;
  if (stacked.length !== n * 3) throw new Error('Plane stack size mismatch');
...
function encodeBlockAdaptive(width: number, height: number, rgb: Uint8Array, dirBits: 8 | 9): Uint8Array {
  const block = 16;
  ...
  for (let by = 0; by < blocksY; by++) {
    for (let bx = 0; bx < blocksX; bx++) {
      ...
      let best = vecPayload;
      if (cls === 'edge' && classicPayload.length <= best.length) best = classicPayload;
      if (classicPayload.length < best.length) best = classicPayload;
      if (planePayload.length < best.length) best = planePayload;
      if (cls === 'noise' && rawPayload.length < best.length) best = rawPayload;
 ...
 */
export function encodeVectorLossless(opts: VectorEncodeOptions): Uint8Array {
  const { width, height, rgb } = opts;
  const dirBits = opts.dirBits ?? 8;
  if (dirBits !== 8 && dirBits !== 9) throw new Error('dirBits must be 8 or 9');
...
export function decodeVectorLossless(payload: Uint8Array, width: number, height: number): VectorDecoded {
  if (payload.length < 1) throw new Error('Truncated vector payload');
  let o = 0;
  const mode = payload[o++]!;
  if (mode === 1) {
    if (o + 4 > payload.length) throw new Error('Truncated vector fallback header');
    const compLen = readU32BE(payload, o);
    o += 4;
    if (o + compLen > payload.length) throw new Error('Truncated vector fallback payload');
  ...
  if (mode === 4) {
    if (o + 4 > payload.length) throw new Error('Truncated plane-stack header');
    const compLen = readU32BE(payload, o);
    o += 4;
    if (o + compLen > payload.length) throw new Error('Truncated plane-stack payload');
    const filtered = entropyDecode(payload.subarray(o, o + compLen));
    const rgb = decodePlaneStack(width, height, filtered);
    return { width, height, rgb, dirBits: 0 };
  }
  if (mode === 2) {
    if (payload.length < 5 + 8) throw new Error('Truncated block-adaptive payload');
    ...
    for (let by = 0; by < blocksY; by++) {
      for (let bx = 0; bx < blocksX; bx++) {
        if (o + 2 + 4 > payload.length) throw new Error('Truncated block header');
        const bw = payload[o++]!;
        const bh = payload[o++]!;
        const len = readU32BE(payload, o);
        o += 4;
        if (o + len > payload.length) throw new Error('Truncated block payload');
        const blkPayload = payload.subarray(o, o + len);
        o += len;
        let blk: Uint8Array;
        const blkMode = blkPayload[0]!;
        if (blkMode === 3) {
          if (blkPayload.length < 5) throw new Error('Truncated raw block');
  ...
  if (mode !== 0) throw new Error('Unsupported vector payload mode');
  if (payload.length < 5 + 16) throw new Error('Truncated vector payload');
  const dirBits = payload[o++]!;
  const lFrac = payload[o++]!;
  o += 2; // reserved
  if ((dirBits !== 8 && dirBits !== 9) || lFrac !== 6) throw new Error('Unsupported vector profile settings');
  ...
  if (end > payload.length) throw new Error('Truncated vector stream');
  ...
  const codebook = buildCodebook(dirBits as 8 | 9);
