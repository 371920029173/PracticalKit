 ...
 */
export function encodeRgbV3dVideo(opts) {
...
export function decodeRgbV3dVideo(file) {
    if (file.length < MAGIC_RGBV3DV.length + 12)
        throw new Error('Truncated .rgbv3dv');
    for (let i = 0; i < MAGIC_RGBV3DV.length; i++) {
        if (file[i] !== MAGIC_RGBV3DV[i])
            throw new Error('Bad .rgbv3dv magic');
    }
    ...
    for (let i = 0; i < frameCount; i++) {
        ...
        o += 4;
        if (o + len > file.length)
            throw new Error('Truncated frame data');
...
function diffToBytes(curr, prev) {
...
function bytesToDiff(diffRaw, prev) {
    if (diffRaw.length !== prev.length * 2)
        throw new Error('Inter-frame diff size mismatch');
...
function computeSadAgainstPredictor(curr, pred, width, x0, y0, bw, bh, srcX0, srcY0) {
    let sad = 0;
    for (let y = 0; y < bh; y++) {
        const rowOff = ((y0 + y) * width + x0) * 3;
        const predOff = ((srcY0 + y) * width + srcX0) * 3;
        for (let x = 0; x < bw * 3; x++)
            sad += Math.abs(curr[rowOff + x] - pred[predOff + x]);
    }
    return sad;
}
function computeSadMotion(curr, prev, width, height, x0, y0, bw, bh, dx, dy) {
    let sad = 0;
    for (let y = 0; y < bh; y++) {
        for (let x = 0; x < bw; x++) {
            const cx = x0 + x;
...
function encodeInterFrameBlockDiff(curr, prev, width, height) {
    const blockSize = 16;
    ...
    for (let by = 0; by < blocksY; by++) {
        for (let bx = 0; bx < blocksX; bx++, bi++) {
            ...
            if (bx > 0)
                leftSad = computeSadAgainstPredictor(curr, currRecon, width, x0, y0, bw, bh, x0 - bw, y0);
            if (by > 0)
                topSad = computeSadAgainstPredictor(curr, currRecon, width, x0, y0, bw, bh, x0, y0 - bh);
            let mvDx = 0;
            let mvDy = 0;
            ...
            modeMap[bi] = mode;
            if (mode === 4)
                motionPairs.push((mvDx + 128) & 0xff, (mvDy + 128) & 0xff);
            for (let y = 0; y < bh; y++) {
                const rowOff = ((y0 + y) * width + x0) * 3;
                for (let x = 0; x < bw * 3; x++) {
...
function decodeInterFrameBlockDiff(prev, width, height, blockSize, modeComp, mvComp, diffComp) {
    const blocksX = Math.ceil(width / blockSize);
    const blocksY = Math.ceil(height / blockSize);
    const curr = new Uint8Array(prev);
    const modeMap = entropyDecode(modeComp);
    if (modeMap.length !== blocksX * blocksY)
        throw new Error('Inter-frame mode map size mismatch');
    ...
    for (let by = 0; by < blocksY; by++) {
        for (let bx = 0; bx < blocksX; bx++, bi++) {
            ...
            let mvDy = 0;
            if (mode === 4) {
                if (mvOff + 2 > mvRaw.length)
                    throw new Error('Truncated motion vector stream');
...
export function decodeRgbV3dVideoInter(file) {
    if (file.length < MAGIC_RGBV3DVI.length + 25)
        throw new Error('Truncated .rgbv3dvi');
    for (let i = 0; i < MAGIC_RGBV3DVI.length; i++) {
        if (file[i] !== MAGIC_RGBV3DVI[i])
            throw new Error('Bad .rgbv3dvi magic');
    }
    let o = MAGIC_RGBV3DVI.length;
    const version = file[o++];
    if (version !== INTER_STREAM_VERSION)
        throw new Error(`Unsupported .rgbv3dvi version: ${version}`);
    const frameCount = readU32BE(file, o);
    o += 4;
    const fpsNumerator = readU32BE(file, o);
    o += 4;
    ...
    o += 4;
    if (o + keyLen > file.length)
        throw new Error('Truncated keyframe');
    const key = decodeRgbV3d(file.subarray(o, o + keyLen));
    o += keyLen;
    const frames = [key.rgb];
    let prev = key.rgb;
    for (let i = 1; i < frameCount; i++) {
        if (o + 1 > file.length)
            throw new Error('Truncated inter frame kind');
        const kind = file[o++];
        let curr;
        if (kind === 1) {
            if (o + 4 > file.length)
                throw new Error('Truncated intra frame length');
            const len = readU32BE(file, o);
            o += 4;
            if (o + len > file.length)
                throw new Error('Truncated intra frame payload');
            curr = decodeRgbV3d(file.subarray(o, o + len)).rgb;
            o += len;
        }
        else if (kind === 0) {
            if (o + 1 + 4 + 4 + 4 > file.length)
                throw new Error('Truncated inter frame header');
            ...
            o += 4;
            if (o + modeLen + mvLen + diffCompLen > file.length)
                throw new Error('Truncated inter frame payload');
