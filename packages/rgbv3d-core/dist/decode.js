...
export function decodeRgbV3d(file) {
    if (!startsWith(file, MAGIC_RGBV3D))
        throw new Error('Not a .rgbv3d v file (bad magic)');
    let o = MAGIC_RGBV3D.length;
    if (o + 4 + 4 + 2 + 4 > file.length)
    ...
    o += 4;
    if (o + compLen > file.length)
        throw new Error('Truncated compressed payload');
    ...
    if (profile === PROFILE_LOSSLESS_RGB_SUB_DEFLATE) {
        ...
        if (filtered.length !== expect)
            throw new Error(`Inflated size mismatch: ${filtered.length} vs ${expect}`);
